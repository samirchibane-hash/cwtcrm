...
import { Package, Building2, Truck, FileText, Save, Calendar, Hash, Tag, DollarSign, Plus, Trash2, AlertTriangle } from 'lucide-react';
...
const OrderDetail = ({ orderId, variant = 'page', onDeleted, linkCustomer = true }: OrderDetailProps) => {
  ...
  // Identity header (title, customer, status) + edit/save/delete actions
  const header = (
    <div className={isPanel ? 'flex flex-col gap-4 pr-8' : 'flex items-start justify-between'}>
      <div className="flex items-center gap-4">
        <div className={`${isPanel ? 'w-12 h-12 rounded-xl' : 'w-14 h-14 rounded-2xl'} bg-muted flex items-center justify-center shrink-0`}>
          <Package className={isPanel ? 'w-6 h-6 text-muted-foreground' : 'w-7 h-7 text-muted-foreground'} />
        </div>
        <div>
          <h1 className={`${isPanel ? 'text-xl' : 'text-2xl'} font-semibold tracking-tight`}>Order #{formData.id}</h1>
  ...
  const content = (
    <div className={gridClass}>
      ...
      {/* Links & Tracking */}
      <section className={`content-card p-6 animate-fade-in ${isPanel ? '' : 'lg:col-span-2'}`} style={{ animationDelay: '150ms' }}>
        <h2 className="section-header">Links & Tracking</h2>
        <div className={`grid grid-cols-1 gap-4 ${isPanel ? '' : 'md:grid-cols-2'}`}>
          <div>
      ...
      {/* Order Updates / Notes */}
      <section className={`content-card p-6 animate-fade-in ${isPanel ? '' : 'lg:col-span-2'}`} style={{ animationDelay: '200ms' }}>
  ...
  if (isPanel) {
    return (
      <div className="flex h-full flex-col">
        <div className="shrink-0 pb-6 border-b border-border">{header}</div>
