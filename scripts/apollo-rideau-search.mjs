#!/usr/bin/env node
/**
 * Search Apollo for Rideau Supply contacts by role/seniority filters,
 * enrich via people/match, verify emails with Clearout,
 * and save to the Rideau Supply prospect in Supabase CRM.
 *
 * Usage: node scripts/apollo-rideau-search.mjs
 */
...
const PROSPECT_ID  = '1260e8a7-4a3b-404a-be9e-50aec1684023'; // Rideau Supply
const ORG_ID       = '604a43d88e6df00001c09dd5'; // Apollo org ID for Rideau Supply
...
// --- Main ---
console.log('\nStep 1: Searching Apollo for Rideau Supply contacts...\n');
...
// Filter out irrelevant roles, cap at TARGET_COUNT
...
console.log(`After role filter (capped at ${TARGET_COUNT}): ${relevant.length} contacts to enrich\n`);
...
if (toAdd.length === 0) {
  ...
  if (saved) {
    console.log(`Added ${toAdd.length} contact(s) to Rideau Supply:\n`);
    toAdd.forEach(c => {
