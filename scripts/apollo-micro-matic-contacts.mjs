#!/usr/bin/env node
/**
 * Search Apollo for Micro Matic contacts, enrich via people/match,
 * verify emails with Clearout, and save to Supabase CRM.
 *
 * Usage: node scripts/apollo-micro-matic-contacts.mjs
 */
...
const PROSPECT_ID  = 'ac1bd268-3e4e-402e-979c-aab33d5d2aa5';
const DOMAIN       = 'micromatic.com';
const MICRO_MATIC_ORG_ID = '54a128fa69702d9ebc094201'; // Micro Matic US - Beverage Dispensing
...
// Standard role filters per CWT workflow
...
// Roles to skip — not relevant to CWT outreach
...
async function apolloSearch(page = 1) {
  const res = await fetch('https://api.apollo.io/v1/mixed_people/api_search', {
    ...
    body: JSON.stringify({
      organization_ids: [MICRO_MATIC_ORG_ID],
...
// --- Supabase ---
// NOTE: The prospects table has RLS enabled. REST API calls require a service role key.
// If SUPABASE_SERVICE_ROLE_KEY is not in .env, the script will output JSON for manual
// DB update via the Supabase MCP (mcp__supabase__execute_sql).
...
// --- Main ---
console.log(`\nStep 1: Searching Apollo for Micro Matic contacts (${DOMAIN})...\n`);
...
// Step 2: Enrich each via people/match
console.log('Step 2: Enriching via Apollo people/match (full name + email + LinkedIn)...\n');
...
for (const person of relevant) {
  ...
  try {
    ...
    if (full) {
      ...
      console.log(`✓ ${full.name} | ${full.email || 'no email'} | ${full.linkedin_url ? 'LinkedIn' : 'no LinkedIn'}`);
    } else {
...
// Step 4: Build contact objects — only save contacts with emails
...
if (toAdd.length === 0) {
  ...
  if (saved) {
    console.log(`\nAdded ${toAdd.length} contact(s) to Micro Matic:\n`);
    toAdd.forEach(c => {
      const verified = c.emailVerified ? '✅ valid' : '⚠️  catch-all/unverified';
      console.log(`  + ${c.name} | ${c.role}`);
      console.log(`    ${c.email} [${verified}]`);
      if (c.linkedIn) console.log(`    ${c.linkedIn}`);
    });
  } else {
    console.log('\nSUPABASE_SERVICE_ROLE_KEY not in .env — DB save skipped.');
    console.log('Run this SQL via Supabase MCP to save contacts:\n');
...
console.log('\nDone. Run scripts/draft-micro-matic-intro.mjs to create Gmail drafts.');
