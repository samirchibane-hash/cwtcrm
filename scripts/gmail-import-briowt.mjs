#!/usr/bin/env node
/**
 * Scrape Gmail for all @briowt.com contacts we've emailed, enrich via Apollo,
 * verify with Clearout, then import into Supabase.
 *
 * Usage: node scripts/gmail-import-briowt.mjs [--dry-run]
 */
...
const DOMAIN       = 'briowt.com';
const PROSPECT_ID  = '97cc24f8-8d51-4afc-886f-00eaf89f58b7';
...
async function scrapeGmailContacts(token) {
  ...
  const emailMap = new Map(); // email → { name, internalDate }
...
async function getExistingContacts() {
  ...
  if (!Array.isArray(rows) || !rows[0]) throw new Error('Brio Water Technology not found in Supabase');
...
console.log('\nStep 4: Looking up Brio Water Technology in Supabase...');
...
if (saved) {
  console.log(`  Saved ${toAdd.length} contact(s) to Brio Water Technology.`);
} else {
  console.log('\nNo service role key — use the SQL above to apply manually.');
}
console.log('\nDone.');
